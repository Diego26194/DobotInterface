...
class ControladorCobot:
    def __init__(self):
        ...
        self.TIMEOUT_PLANIFICACION = 2.0    # segundos máximos de espera
    ...
    def callback_planificacion(self, msg):
        ...
        if self.orden==0:
            ...
            # Esperar feedback y trayectorias planificadas
            inicio = time.time()
            while (not self.recibido_waits) and (not self.recibido_trayectorias) and (time.time() - inicio < self.TIMEOUT_PLANIFICACION):
                rospy.sleep(0.1)
    ...
    def ejecutar_plan_completo(self):
        ...
        for i, frag in enumerate(self.plan_fragmentos):
            ...
            if (i) in self.indicesT:
                self.ejecutar_trayectoria_bruta(self.trayectoras[i])               
